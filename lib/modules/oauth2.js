...
everyModule.submodule('oauth2')
  ...
  .getAuthUri( function (req, res, next) {
    ...
    var params = {
      ...
      , unescapedParams = this.unescapedAuthQueryParams
      , param;
    ...
    // Add unescaped params
    if (unescapedParams) for (var i in unescapedParams) {
      url += '&' + i + '=' + unescapedParams[i];
    }

    // Return our complete URL
    return url;
  })
...
oauth2.unescapedAuthQueryParams = {};
oauth2.moreAccessTokenParams = {};
oauth2.moreAccessTokenQueryParams = {};
oauth2.cloneOnSubmodule.push('moreAuthQueryParams', 'unescapedAuthQueryParams', 'moreAccessTokenParams', 'moreAccessTokenQueryParams');
...
// Some services take unescaped params that include things like '+'
// cough Hubspot
// use this with caution and only on values you can control in the URL
oauth2.unescapedAuthQueryParams = function(key, val) {
  if (arguments.length === 1 && key.constructor === Object) {
    for (var k in key) {
      this.unescapedAuthQueryParams(k, key[k]);
    }
    return this;
  }
  if (val)
    this.unescapedAuthQueryParams[key] = val;
  return this;
};
